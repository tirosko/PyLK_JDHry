...
# ciara=c.create_line(0,0,5,25,50,100,fill="yellow")
# for i in range(10):
#     mierka=2*(i+1)
#     lod_id = c.create_polygon(mierka*5, mierka*5, mierka*5, mierka*25, mierka*30, mierka*15, fill="red")
#     c.update()
#     time.sleep(1)
lod_id = c.create_polygon(5, 5, 5, 25, 30, 15, fill="red")
# lod_id = c.create_polygon(10, 4, 4, mierka*2, mierka*2, mierka*2, fill="red")
...
def hyb_lod(udalost):
    if udalost.keysym == "Up":
        ...
        c.move(lod_id, LOD_RYCH, 0)
        c.move(lod_id2, LOD_RYCH, 0)
    elif udalost.keysym == "KeyPress-a":
        # Ponorka sa po stlačení šípky vľavo hýbe vľavo
        c.move(lod_id, -LOD_RYCH, 0)
        c.move(lod_id2, -LOD_RYCH, 0)
